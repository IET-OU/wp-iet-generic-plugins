...
window.jQuery(function ($) {
  ...
  var W = window;
  var debug = W.location.search.match(/debug=1/);
  var C = debug && W.console;
  var $bodyClass = $('#lace-js-body-class');
  var has_cleanprint = W.WpCpCleanPrintPrintHtml || W.CleanPrintHtml;
  var $inject_cleanprint = $('body'); // .search-results, body.single-hypothesis")
  var cleanprint_post_re = /(search-results|postid-\d+)/;
  var cleanprint_exclude_sel = '#cookie-notice, .nav-menu, .assistive-text, .lace-assistive-help, .jxl-message, .oer-chart-loading, .leaflet-control-container, .hyp-tpl-hint';
  var cleanprint_include_sel = '.page-title, .entry-title, #X-primary';
  var vis_table_row_sel = '.hypothesis #country-vis-table tbody tr';

  C && console.log('Body class:', $bodyClass);
  $('body').addClass($bodyClass && $bodyClass.attr('class'));
  ...
  */
  ...
  var banner_path = '/wp-content/plugins/wp-iet-generic-plugins/images/lace/lace';
  var banner_alt = 'Evidence Hub - learning analytics community exchange (LACE)';
  var with_banner = $('.lace-with-banner').length;
  ...
  */
    ...
    */
    $cleanprint_bn_wrap.find('a, button').on('click', function ( /* ev */ ) {
      ...
      whenCall(function () {
        ...
        var $cp_dialog = $('#cpf-root iframe:first');
  ...
  function whenCall (whenTrueFN, callbackFN, interval) {
